"""Root-level ASGI export.\n\nVercel's zero-config detection looks here first (it chose this file over\n``api/index.py`` on the first deploy), and ``uvicorn app:app`` expects it too.\nIt shares :mod:`src.asgi` with ``api/index.py`` so whichever entry point the\nplatform picks, the behaviour is identical.\n\nThis module must never raise at import: if it does, the platform returns an\nopaque FUNCTION_INVOCATION_FAILED with no indication of the cause."""
